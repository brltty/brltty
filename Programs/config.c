...
changeKeyboardKeyTable (const char *name) {
  ...
  if ((keyboardKeyTable = table)) {
    ...
    setLogKeyEventsFlag(keyboardKeyTable, &LOG_CATEGORY_FLAG(KEYBOARD_KEY_EVENTS));
...
static void
retryKeyboardMonitor (const AsyncAlarmResult *result UNUSED) {
          ...
          if (file) {
            ...
            if ((path = makePath(opt_tablesDirectory, file))) {
              if ((brl.keyTable = compileKeyTable(path, brl.keyNameTables))) {
                ...
                setLogKeyEventsFlag(brl.keyTable, &LOG_CATEGORY_FLAG(BRAILLE_KEY_EVENTS));
...
static void
retryBrailleDriver (const AsyncAlarmResult *result UNUSED) {
...
static void
retrySpeechDriver (const AsyncAlarmResult *result UNUSED) {
...
static void
retryScreenDriver (const AsyncAlarmResult *result UNUSED) {
...
static void
retryPidFile (const AsyncAlarmResult *result UNUSED) {
  tryPidFile();
}
