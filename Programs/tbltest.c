...
writeTable_Native (const char *path, FILE *file, const TranslationTable table, void *data) {
  ...
  {
    const char *charset = tblGetCharset();
    if (charset)
...
");
  ...
  for (i=0; i<=32; i++)
    fprintf(file, "UNICODE-CHAR U+%04x U+28%02x\n", i, table[i]);

  for (i=33; i<256; i++) {
    char c = i;
    wchar_t wc;
    wchar_t pattern = 0x2800 | table[i];
    char utf8C[MB_LEN_MAX + 1], utf8Pattern[MB_LEN_MAX + 1];

    if ((wc = tblCharToWchar(c)) == WEOF)
      continue;
    if (!tblWcharToUtf8(wc, utf8C))
      continue;
    if (!tblWcharToUtf8(pattern, utf8Pattern))
      continue;
    fprintf(file, "UCS-CHAR %s %s\n", utf8C, utf8Pattern);
  }
  return 1;
}
#endif /* HAVE_ICONV_H */
...
main (int argc, char *argv[]) {
  ...
  if (opt_characterSet && !tblSetCharset(opt_characterSet)) {
