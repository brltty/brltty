...
");
  ...
  for (i=0; i<=0XFF; i++) {
    ...
    if ((wc = tblCharToWchar(c)) == WEOF) continue;
    if (isprint(i) && !isspace(i)) {
      char utf8C[MB_LEN_MAX + 1], utf8Pattern[MB_LEN_MAX + 1];
      if (!tblWcharToUtf8(wc, utf8C)) continue;
      if (!tblWcharToUtf8(pattern, utf8Pattern)) continue;
      fprintf(file, "UCS-CHAR %s %s\n", utf8C, utf8Pattern);
    } else {
      uint32_t u32 = pattern;
      fprintf(file, "UNICODE-CHAR U+%04x U+%04"PRIx32"\n", i, u32);
    }
  }
  return 1;
}
