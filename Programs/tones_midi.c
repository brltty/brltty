...
static int openMidi (void) {
   if (fileDescriptor == -1) {
      if ((fileDescriptor = getMidiDevice(flushBuffer)) == -1) {
         LogPrint(LOG_WARNING, "Cannot open MIDI.");
         return 0;
      }
      setCloseOnExec(fileDescriptor);
      LogPrint(LOG_DEBUG, "MIDI opened: fd=%d", fileDescriptor);
   }
   setMidiInstrument(deviceNumber, channelNumber, prefs.midiinstr);
   return 1;
}

static int generateMidi (int note, int duration) {
   if (fileDescriptor != -1) {
      beginMidiBlock(fileDescriptor);
      if (note) {
	 LogPrint(LOG_DEBUG, "Tone: msec=%d note=%d", duration, note);
         startMidiNote(deviceNumber, channelNumber, note);
	 insertMidiWait(duration);
         stopMidiNote(deviceNumber, channelNumber, note);
      } else {
...
const ToneGenerator midiToneGenerator = {
   openMidi,
   generateMidi,
   flushMidi,
   closeMidi
