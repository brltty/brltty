...
{
  return brlapi_getParameterAlloc(BRLAPI_PARAM_KEY_SHORT_NAME, (key & ~BRLAPI_KEYCODE_PRESS), BRLAPI_PARAMF_GLOBAL, NULL);
}
...
static void showKeyCodes(void)
{
  ...
  while ((res = brlapi_readKey(1, &key)) != -1) {
    const char *action = (key & BRLAPI_KEYCODE_PRESS)? "press": "release";
