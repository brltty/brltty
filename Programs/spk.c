...
static size_t
formatTime (char *buffer, size_t size, uint8_t hour, uint8_t minute, uint8_t second) {
  size_t length;
  ...
  switch (prefs.timeFormat) {
    ...
    case tf12Hour: {
      ...
      oclock = gettext("oclock");
      suffix = (hour < twelve)? " a m": " p m";
...
static size_t
formatDate (char *buffer, size_t size, uint16_t year, uint8_t month, uint8_t day) {
  size_t length;
...
sayTime (
  ...
  formatTime(time, sizeof(time), hour, minute, second);

  if (prefs.datePosition == dpNone) {
    STR_PRINTF("%s", time);
  } else {
    char date[0X40];
    formatDate(date, sizeof(date), year, month, day);
