...
/* Vario/brl.c - Braille display driver for the Baum Vario 40 and 80 displays
 *
...
#define KEYAND(code) \
  ...
  if (!read (brl_fd, buf, 1)){
    if (button_waitcount >= BUTTON_STEP) { 
      ...
      button_waitcount++;
    else if (elapsed_msec(&lastcmd_time, &now) > REPEAT_TIME &&
	     is_repeat_cmd(lastcmd)) {
      gettimeofday(&lastcmd_time, &dum_tz);
      return(lastcmd);
    }
#ifdef USE_PING
    else if ((i = elapsed_msec(&last_ping, &now) > PING_INTRVL)){
